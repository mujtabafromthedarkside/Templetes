...
if not os.path.exists('temp'):
    os.mkdir('temp')
create_video_from_image(imgName, 10, 30, temp_outputs[-1])

random_number += 1
temp_outputs.append(f'temp/{random_number}.mp4')
apply_blinking_effect(temp_outputs[-2], 1,4, temp_outputs[-1])

random_number += 1
temp_outputs.append(f'temp/{random_number}.mp4')
tv_filter(temp_outputs[-2], 6,8, temp_outputs[-1])
